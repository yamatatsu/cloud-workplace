...
VPS 上に LibreChat（+ MongoDB/Redis）と OpenCode 実行環境を展開する。展開方式の選択が必要。
...
LibreChat およびその依存（MongoDB、Redis）は Docker Compose でコンテナ化して展開する。OpenCode と herdr はコンテナではなく VPS 上のプロセスとして実行する（シェル/ファイルシステムを直接扱うため）。
...
- マイナス: メモリ 2GB 程度では MongoDB(+Redis) のチューニングが必要になる可能性がある（残課題として追跡）。
